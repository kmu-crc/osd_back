...
exports.designInDesigner = (req, res, next) => {
  ...
  let sql = "SELECT D.uid, D.user_id, D.title, D.thumbnail, D.category_level1, D.category_level2, D.create_time, C.like_count, C.member_count, C.card_count, C.view_count FROM design_member M JOIN design D ON D.uid = M.design_id AND is_public = 0 LEFT JOIN design_counter C ON C.design_id = D.uid WHERE M.user_id = " + id;
  req.sql = sql;
  next();
};
