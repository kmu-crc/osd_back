...
exports.createContents = (user_id, card_id, list) => {
  console.log("create content db");
  ...
  new Promise((resolve, reject) => {
    ...
    const insertDB = async arr => {
      return new Promise(async (resolve, reject) => {
        ...
        for (let item of arr) {
          let obj = {
            // user_id, card_id, content, content_type, data_type, extension, order, file_name //
          ...
          await connection.query("INSERT INTO market.content SET ?", obj, (err, rows) => {
            if (!err) {
...
exports.createCardDB = data => {
  console.log("createCardDB", data);
  return new Promise((resolve, reject) => {
    connection.query("INSERT INTO market.card SET ?", data, (err, rows) => {
      if (!err) {
...
exports.updateItemContent = (req, res, next) => {
  ...
  const EditContents = list => {
    return new Promise((resolve, reject) => {
      // console.log("Edit Contents:", list, list == null || list.length === 0);
      if (list == null || list.length === 0) resolve(true);
      Promise.all(list.map(ele => editContent(ele)))
        .then(resolve(true))
        .catch(err => reject(err));
    });
  };
  const deleteContent = data => {
    return new Promise((resolve, reject) => {
      const sql = `DELETE FROM market.content WHERE uid=${data.uid}`;
  ...
  const DeleteContents = list => {
    return new Promise((resolve, reject) => {
      if (list == null | list.length === 0) resolve(true);
      Promise.all(list.map(ele => deleteContent(ele)))
        .then(resolve(true))
        .catch(err => reject(err));
    });
  };
  ...
  const respond = () => {
    res.status(200).json({ success: true, message: "아이템 수정 완료" });
  };
  const error = (err) => {
    res.status(500).json({ success: false, message: err });
  };

  EditContents(req.body.updateContent)
    .then(this.createContents(req.decoded.uid, req.params.card, req.body.newContent))
    .then(DeleteContents(req.body.deleteContent))
    .then(respond)
    .catch(error);
};


exports.deleteItemCard = (req, res, next) => {
  const card_id = req.params.card_id;

  const deleteCard = (id) => {
    return new Promise((resolve, reject) => {
      const sql = `DELETE FROM market.card WHERE uid=${id}`;
  ...
  deleteCard(card_id)
    .then(res.status(200).json({ success: true, message: "카드가 삭제되었습니다." }))
    .catch(err => res.status(500).json({ success: true, message: err }))
};
